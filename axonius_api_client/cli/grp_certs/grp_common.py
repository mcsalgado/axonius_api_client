...
from ...setup_env import ENV_KEYS, set_env
...
OPT_UPDATE_ENV = click.option(
    ...
    "update_env",
    default=False,
    help=f"Add '{ENV_KEYS.CERTPATH}={{path_to_leaf_cert}}' to {INIT_DOTENV!r}",
    is_flag=True,
    show_envvar=True,
    show_default=True,
)
...
def handle_update_env(update_env, export_file):
    """Pass."""
    entry = f'{ENV_KEYS.CERTPATH}="{export_file}"'
    uenv = f"update_env={update_env}"
    if update_env:
        set_env(key=ENV_KEYS.CERTPATH, value=export_file)
        click.secho(
